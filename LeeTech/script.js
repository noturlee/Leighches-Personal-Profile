...
var typed = new Typed(".typing-1", {
    strings: ["Designer", "Developer", "Thinker", "Winner", "Pokéfan"],
    typeSpeed: 80,
    backSpeed: 60,
    loop: true                    
});

typed.start(); // Add this line to start the typing animation

function speakHello() {
  const synth = window.speechSynthesis;
  const utterance = new SpeechSynthesisUtterance('Lee-Shay Jaikarran');

  // Use the `getVoices` method to retrieve an array of available voices
  const voices = synth.getVoices();

  // Manually select an English voice if available
...
document.querySelectorAll('.gallery-cell').forEach(function(cell) {
    cell.addEventListener('click', function() {
        // Expand the image or perform any other action
        alert('For security purposes, You can view my certificates on LinkedIn');
    });
});
